    ...
    @IBAction func difficultyAction(_ sender: UISegmentedControl) {
        ...
        case 1: if isDownloadingSecond == false {
            ...
            databaseRef.child("\(gym)Route").child("V2V3").observe(.childAdded, with: { (snapshot) in
                ...
                if let requestData = snapshot.value as? [String: String] {
                    self.V2V3.append(routeInfo(area: requestData["area"]!, color: requestData["color"]!, difficulty: requestData["difficulty"]!, points: requestData["points"]!, image: requestData["imageURL"]!, autoID: snapshot.key, creator: requestData["creator"]!, creatorName: requestData["creatorName"]!, imageUUID: requestData["imageUUID"]!))
        ...
        case 2: if isDownloadingThird == false {
            ...
            databaseRef.child("\(gym)Route").child("V4V5").observe(.childAdded, with: { (snapshot) in
                self.isDownloadingFirst = true
                if let requestData = snapshot.value as? [String: String] {
                    self.V4V5.append(routeInfo(area: requestData["area"]!, color: requestData["color"]!, difficulty: requestData["difficulty"]!, points: requestData["points"]!, image: requestData["imageURL"]!, autoID: snapshot.key, creator: requestData["creator"]!, creatorName: requestData["creatorName"]!, imageUUID: requestData["imageUUID"]!))
                    self.difficultyTableView.reloadData()
        ...
        case 3: if isDownloadingFourth == false {
            ...
            databaseRef.child("\(gym)Route").child("V6V7").observe(.childAdded, with: { (snapshot) in
                self.isDownloadingFirst = true
                if let requestData = snapshot.value as? [String: String] {
                    self.V6V7.append(routeInfo(area: requestData["area"]!, color: requestData["color"]!, difficulty: requestData["difficulty"]!, points: requestData["points"]!, image: requestData["imageURL"]!, autoID: snapshot.key, creator: requestData["creator"]!, creatorName: requestData["creatorName"]!, imageUUID: requestData["imageUUID"]!))
                    self.difficultyTableView.reloadData()
...
//        }
        ...
        databaseRef.child("\(gym)Route").child("V0V1").observe(.childAdded, with: { (snapshot) in
            self.isDownloadingFirst = true
            if let requestData = snapshot.value as? [String: String] {
                self.V0V1.append(routeInfo(area: requestData["area"]!, color: requestData["color"]!, difficulty: requestData["difficulty"]!, points: requestData["points"]!, image: requestData["imageURL"]!, autoID: snapshot.key, creator: requestData["creator"]!, creatorName: requestData["creatorName"]!, imageUUID: requestData["imageUUID"]!))
                self.difficultyTableView.reloadData()
            ...
            if difficultySegment.selectedSegmentIndex == 0 {
                if V0V1[indexPath.row].creator == currentUser!.email {
                    ...
                    let okAction = UIAlertAction(title: "刪除", style: .destructive, handler: { (UIAlertAction) in
                        ...
                        Database.database().reference().child("video").child(self.V0V1[indexPath.row].autoID).observeSingleEvent(of: .value, with: { (DataSnapshot) in
                            ...
                            for element in deleteKey {
                                ...
                                storageRef.child(self.V0V1[indexPath.row].autoID).child("\(element).mp4").delete(completion: { (error) in
                                    if let error = error {
                                        print("\(error)")
                                    }})
                            }
                            Storage.storage().reference().child("\(self.V0V1[indexPath.row].imageUUID).png").delete(completion: { (error) in
                                
                            })
                            
                            Database.database().reference().child("\(self.gym)Route").child("V0V1").child(self.V0V1[indexPath.row].autoID).removeValue()
                            Database.database().reference().child("video").child(self.V0V1[indexPath.row].autoID).removeValue()
                ...
                if V2V3[indexPath.row].creator == currentUser!.email {
                    ...
                    Database.database().reference().child("video").child(self.V2V3[indexPath.row].autoID).observeSingleEvent(of: .value, with: { (DataSnapshot) in
                            ...
                            for element in deleteKey {
                                let storageRef = Storage.storage().reference()
                                storageRef.child(self.V2V3[indexPath.row].autoID).child("\(element).mp4").delete(completion: { (error) in
                                    if let error = error {
                                        print("\(error)")
                                    }})
                            }
                        Storage.storage().reference().child("\(self.V2V3[indexPath.row].imageUUID).png").delete(completion: { (error) in
                            
                        })
                            Database.database().reference().child("\(self.gym)Route").child("V2V3").child(self.V2V3[indexPath.row].autoID).removeValue()
                            Database.database().reference().child("video").child(self.V2V3[indexPath.row].autoID).removeValue()
                            self.stopAnimating()
                ...
                if V4V5[indexPath.row].creator == currentUser!.email {
                    ...
                    let okAction = UIAlertAction(title: "OK", style: .destructive, handler: { (UIAlertAction) in
                        ...
                        Database.database().reference().child("video").child(self.V4V5[indexPath.row].autoID).observeSingleEvent(of: .value, with: { (DataSnapshot) in
                                                       if let requestData = DataSnapshot.value as? [String:Any] {
                                for element in requestData {
                                    deleteKey.append(element.key)
                                }
                            ...
                            for element in deleteKey {
                                let storageRef = Storage.storage().reference()
                                storageRef.child(self.V4V5[indexPath.row].autoID).child("\(element).mp4").delete(completion: { (error) in
                                    if let error = error {
                                        print("\(error)")
                                    }})
                            }
                            Storage.storage().reference().child("\(self.V4V5[indexPath.row].imageUUID).png").delete(completion: { (error) in
                                
                            })
                            Database.database().reference().child("\(self.gym)Route").child("V4V5").child(self.V4V5[indexPath.row].autoID).removeValue()
                            Database.database().reference().child("video").child(self.V4V5[indexPath.row].autoID).removeValue()
                            self.stopAnimating()
                ...
                if V6V7[indexPath.row].creator == currentUser!.email {
                    ...
                    let okAction = UIAlertAction(title: "OK", style: .destructive, handler: { (UIAlertAction) in
                        var deleteKey: [String] = []
                        self.startAnimating(CGSize.init(width: 120, height: 120),message: "deleting")
                        Database.database().reference().child("video").child(self.V6V7[indexPath.row].autoID).observeSingleEvent(of: .value, with: { (DataSnapshot) in
                            ...
                            for element in deleteKey {
                                let storageRef = Storage.storage().reference()
                                storageRef.child(self.V6V7[indexPath.row].autoID).child("\(element).mp4").delete(completion: { (error) in
                                    if let error = error {
                                        print("\(error)")
                                    }})
                            }
                            Storage.storage().reference().child("\(self.V6V7[indexPath.row].imageUUID).png").delete(completion: { (error) in
                                
                            })
                            Database.database().reference().child("\(self.gym)Route").child("V6V7").child(self.V6V7[indexPath.row].autoID).removeValue()
                            Database.database().reference().child("video").child(self.V6V7[indexPath.row].autoID).removeValue()
                            self.stopAnimating()
    ...
    func deleteNotAllow(message: String) {
        let alertController = UIAlertController(title: message, message: "這不是你上傳的路線", preferredStyle: .alert)
